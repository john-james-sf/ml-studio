...
from ..model_evaluation.model_validation import studentized_residuals
...
def cooks_distance(model, X, y):
    ...
    student_resid, _ = studentized_residuals(model, X, y)
    p = X.shape[1]
    n = X.shape[0]
    # Compute Cooks
    cooks_d = 1/p * n/(n-p) * student_resid**2 * hat / (1-hat)
    return cooks_d
...
def dffits(model, X, y):
    ...
    times \\sqrt(\\frac{h_{ii}}{1-h_{ii}})[1]_
