...
def plot_loss(model, title=None, figsize=(12,4), directory=None, filename=None):
    ...
    if 'val_cost' in model.history.epoch_log:
        fig, _ = _plot_train_val_loss(model, title=title, 
                                            figsize=figsize)
    else:
        fig, _ = _plot_train_loss(model, title=title, 
                                        figsize=figsize)
...
def plot_score(model, title=None, figsize=(12,4), directory=None, filename=None):
    ...
    if 'val_score' in model.history.epoch_log:
        fig, _ = _plot_train_val_score(model, title=title, 
                                            figsize=figsize)
    else:
        fig, _ = _plot_train_score(model, title=title, 
                                        figsize=figsize)
