...
class LinearRegressionTests:
    ...
    def test_sgd_linear_regression_training_solution(self, train_sgd_regression,
                                                 analytical_solution_training_data):
        train_solution = analytical_solution_training_data
        gd, X_train, _, y_train, _ = train_sgd_regression
    ...
    @mark.sgd
    @mark.linear_regression
    def test_sgd_linear_regression_test_scores(self, train_sgd_regression, 
                                           regression_metric):        
        gd, _, X_test, _, y_test = train_sgd_regression
...
class LassoRegressionTests:
    ...
    def test_sgd_lasso_regression_training_solution(self, train_sgd_lasso_regression,
                                                 analytical_solution_training_data):
        train_solution = analytical_solution_training_data
        gd, X_train, _, y_train, _ = train_sgd_lasso_regression
        y_pred = gd.predict(X_train)        
        assert all(np.isclose(gd.theta, train_solution, rtol=1e1)), "Solution is not close to analytical solution."
        assert all(np.isclose(y_train, y_pred, rtol=1e1)), "Train predictions are not close to true values."  
    ...
    @mark.sgd
    @mark.lasso_regression
    def test_sgd_lasso_regression_test_scores(self, train_sgd_lasso_regression, 
                                           regression_metric):        
        gd, _, X_test, _, y_test = train_sgd_lasso_regression
        y_pred = gd.predict(X_test)        
        score = gd.score(X_test, y_test)
        assert all(np.isclose(y_test, y_pred, rtol=1e1)), "Test predictions are not close to true values."  
        if regression_metric == 'r2':
...
class RidgeRegressionTests:
    ...
    def test_sgd_ridge_regression_training_solution(self, train_sgd_ridge_regression,
                                                 analytical_solution_training_data):
        train_solution = analytical_solution_training_data
        gd, X_train, _, y_train, _ = train_sgd_ridge_regression
        y_pred = gd.predict(X_train)        
        assert all(np.isclose(gd.theta, train_solution, rtol=1e1)), "Solution is not close to analytical solution."
        assert all(np.isclose(y_train, y_pred, rtol=1e1)), "Train predictions are not close to true values."  
    ...
    @mark.sgd
    @mark.ridge_regression
    def test_sgd_ridge_regression_test_scores(self, train_sgd_ridge_regression, 
                                           regression_metric):        
        gd, _, X_test, _, y_test = train_sgd_ridge_regression
        y_pred = gd.predict(X_test)        
        score = gd.score(X_test, y_test)
        assert all(np.isclose(y_test, y_pred, rtol=1e1)), "Test predictions are not close to true values."  
        if regression_metric == 'r2':
...
class ElasticNetRegressionTests:
    ...
    def test_sgd_elasticnet_regression_training_solution(self, train_sgd_elasticnet_regression,
                                                 analytical_solution_training_data):
        train_solution = analytical_solution_training_data
        gd, X_train, _, y_train, _ = train_sgd_elasticnet_regression
        y_pred = gd.predict(X_train)        
        assert all(np.isclose(gd.theta, train_solution, rtol=1e1)), "Solution is not close to analytical solution."
        assert all(np.isclose(y_train, y_pred, rtol=1e1)), "Train predictions are not close to true values."  
    ...
    @mark.sgd
    @mark.elasticnet_regression
    def test_sgd_elasticnet_regression_test_scores(self, train_sgd_elasticnet_regression, 
                                           regression_metric):        
        gd, _, X_test, _, y_test = train_sgd_elasticnet_regression
        y_pred = gd.predict(X_test)        
        score = gd.score(X_test, y_test)
        assert all(np.isclose(y_test, y_pred, rtol=1e1)), "Test predictions are not close to true values."  
        if regression_metric == 'r2':
