...
class LinearRegressionTests:
    ...
    def test_linear_regression_validation(self, get_regression_data):
        ...
        with pytest.raises(TypeError):
            ...
            lr.fit(X, y)        
        with pytest.raises(TypeError):
            lr = LinearRegression(batch_size='k')            
            lr.fit(X, y)
        with pytest.raises(TypeError):
            lr = LinearRegression(theta_init='k')            
            lr.fit(X, y)
        with pytest.raises(TypeError):
            lr = LinearRegression(epochs='k')           
            lr.fit(X, y)
        with pytest.raises(ValueError):
            lr = LinearRegression(cost='x')                                
            lr.fit(X, y) 
        with pytest.raises(ValueError):
            lr = LinearRegression(cost=None)                                
            lr.fit(X, y)                  
        with pytest.raises(TypeError):
            lr = LinearRegression(early_stop='x')                                
            lr.fit(X, y)
        with pytest.raises(TypeError):
            lr = LinearRegression(metric=0)                                
            lr.fit(X, y)             
        with pytest.raises(ValueError):
            lr = LinearRegression(metric='x')                                
            lr.fit(X, y) 
        with pytest.raises(ValueError):
            lr = LinearRegression(early_stop=EarlyStopPlateau(), metric=None)                                
            lr.fit(X, y)                                                             
        with pytest.raises(TypeError):
            lr = LinearRegression(epochs=10,verbose=None)                                                                                      
            lr.fit(X, y)
        with pytest.raises(ValueError):
            lr = LinearRegression(epochs=10,checkpoint=-1)
            lr.fit(X, y)
        with pytest.raises(TypeError):
            lr = LinearRegression(epochs=10,checkpoint='x')
            lr.fit(X, y)  
        with pytest.warns(UserWarning):
            lr = LinearRegression(epochs=10,checkpoint=100)
            lr.fit(X, y)                        
        with pytest.raises(TypeError):
            lr = LinearRegression(epochs=10,seed='k')                                
            lr.fit(X, y)
                       

    @mark.linear_regression
    @mark.linear_regression_get_params
    def test_linear_regression_get_params(self):
        lr = LinearRegression(learning_rate=0.01, theta_init=np.array([2,2,2]),
    ...
    @mark.linear_regression
    @mark.linear_regression_validate_data
    def test_linear_regression_validate_data(self, get_regression_data):
        lr = LinearRegression(epochs=10)
        X, y = get_regression_data        
        with pytest.raises(TypeError):
            lr.fit([1,2,3], y)
        with pytest.raises(TypeError):
    ...
    @mark.linear_regression
    @mark.linear_regression_prepare_data
    def test_linear_regression_prepare_data_no_val_set(self, get_regression_data):
        lr = LinearRegression(epochs=10)
        X, y = get_regression_data                
        lr.fit(X,y)
    ...
    @mark.linear_regression
    @mark.linear_regression_prepare_data
    def test_linear_regression_prepare_data_w_val_set(self, get_regression_data):                
        es = EarlyStopPlateau(val_size=0.2)
        lr = LinearRegression(epochs=10, early_stop=es)
        X, y = get_regression_data                
        lr.fit(X,y)
        assert X.shape[1] == lr.X.shape[1] - 1, "intercept column not added to X in prepare data"
    ...
    @mark.linear_regression
    @mark.linear_regression_compile
    def test_linear_regression_compile(self, get_regression_data):
        X, y = get_regression_data        
        lr = LinearRegression(epochs=10)
        lr.fit(X,y)        
    ...
    @mark.linear_regression
    @mark.linear_regression_init_weights
    def test_linear_regression_init_weights_shape_match(self, get_regression_data):        
        X, y = get_regression_data        
        theta_init = np.ones(X.shape[1]+1)        
        lr = LinearRegression(epochs=10, theta_init=theta_init)
        lr.fit(X,y)
        assert not all(np.equal(lr.theta, lr.theta_init)), "final and initial thetas are equal" 

    @mark.linear_regression
    @mark.linear_regression_init_weights
    def test_linear_regression_init_weights_shape_mismatch(self, get_regression_data):        
        X, y = get_regression_data        
    ...
    @mark.linear_regression
    @mark.linear_regression_learning_rate
    def test_linear_regression_fit_learning_rate_constant(self, get_regression_data):
        X, y = get_regression_data        
        lr = LinearRegression(learning_rate = 0.1, epochs=10)
        lr.fit(X,y)
    ...
    @mark.linear_regression
    @mark.linear_regression_learning_rate
    def test_linear_regression_fit_learning_rate_sched(self, get_regression_data,
                                                      learning_rate_schedules):
        X, y = get_regression_data        
        lrs = learning_rate_schedules
        lr = LinearRegression(learning_rate = lrs, epochs=100)
        lr.fit(X,y)
        assert lr.history.epoch_log['learning_rate'][0] !=\
            lr.history.epoch_log['learning_rate'][-1], "learning rate not changed in history"

    @mark.linear_regression
    @mark.linear_regression_batch_size
    def test_linear_regression_fit_batch_size(self, get_regression_data):
        X, y = get_regression_data         
        X = X[0:33]
        y = y[0:33]       
        lr = LinearRegression(batch_size=32, epochs=10)
        lr.fit(X,y)                
    ...
    @mark.linear_regression
    @mark.linear_regression_epochs
    def test_linear_regression_fit_epochs(self, get_regression_data):
        X, y = get_regression_data                
        lr = LinearRegression(epochs=10)
        lr.fit(X,y)
    ...
    @mark.linear_regression
    @mark.linear_regression_early_stop
    def test_linear_regression_fit_early_stop(self, get_regression_data,
                                                     early_stop):
        es = early_stop
        X, y = get_regression_data                
        lr = LinearRegression(learning_rate=0.5, epochs=5000, early_stop=es)
        lr.fit(X,y)
    ...
    def test_linear_regression_predict(self, get_regression_data):
        X, y = get_regression_data                
    ...
    def test_linear_regression_score(self, get_regression_data_w_validation, 
                                    regression_metric):
        X, X_test, y, y_test = get_regression_data_w_validation                
        lr = LinearRegression(learning_rate = 0.1, epochs=1000, metric=regression_metric)
    ...
    @mark.linear_regression
    @mark.linear_regression_history
    def test_linear_regression_history_no_val_data(self, get_regression_data):        
        X, y = get_regression_data        
        lr = LinearRegression(epochs=10)
        lr.fit(X, y)        
        # Test epoch history
    ...
    @mark.linear_regression
    @mark.linear_regression_history
    def test_linear_regression_history_w_val_data(self, get_regression_data):        
        X, y = get_regression_data
        es = EarlyStopPlateau(precision=0.1)        
        lr = LinearRegression(epochs=100, metric='mean_squared_error', early_stop=es)
        lr.fit(X, y)        
        # Test epoch history
